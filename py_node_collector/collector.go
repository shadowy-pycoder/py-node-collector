/*
cgo stubs for package collector.
...
#cgo CFLAGS: "-I/usr/include/python3.12" -Wno-error -Wno-implicit-function-declaration -Wno-int-conversion
#cgo LDFLAGS: "-L/usr/lib/x86_64-linux-gnu" "-lpython3.12" -ldl -lm
...
import (
	...
	"github.com/shadowy-pycoder/py-node-collector/collector"
)
...
// --- generated code for package: collector below: ---
...
// Converters for pointer handles for type: *collector.NodeCollector
func ptrFromHandle_Ptr_collector_NodeCollector(h CGoHandle) *collector.NodeCollector {
	p := gopyh.VarFromHandle((gopyh.CGoHandle)(h), "*collector.NodeCollector")
	if p == nil {
		return nil
	}
	return gopyh.Embed(p, reflect.TypeOf(collector.NodeCollector{})).(*collector.NodeCollector)
}
func handleFromPtr_Ptr_collector_NodeCollector(p interface{}) CGoHandle {
	return CGoHandle(gopyh.Register("*collector.NodeCollector", p))
}

// Converters for pointer handles for type: collector.Collector
func ptrFromHandle_collector_Collector(h CGoHandle) collector.Collector {
	p := gopyh.VarFromHandle((gopyh.CGoHandle)(h), "collector.Collector")
	if p == nil {
		return nil
	}
	return p.(collector.Collector)
}
func handleFromPtr_collector_Collector(p interface{}) CGoHandle {
	return CGoHandle(gopyh.Register("collector.Collector", p))
}

// Converters for non-pointer handles for type: collector.NodeCollector
func ptrFromHandle_collector_NodeCollector(h CGoHandle) *collector.NodeCollector {
	p := gopyh.VarFromHandle((gopyh.CGoHandle)(h), "collector.NodeCollector")
	if p == nil {
		return nil
	}
...
func handleFromPtr_collector_NodeCollector(p interface{}) CGoHandle {
	return CGoHandle(gopyh.Register("collector.NodeCollector", p))
}
...
// --- wrapping struct: collector.NodeCollector ---
//
//export collector_NodeCollector_CTor
func collector_NodeCollector_CTor() CGoHandle {
	return CGoHandle(handleFromPtr_collector_NodeCollector(&collector.NodeCollector{}))
}
...
//export collector_Collect
func collector_Collect() *C.char {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	return C.CString(collector.Collect())
